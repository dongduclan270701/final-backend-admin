import { collectingAdminService } from '*/services/Admin/collectingAdmin.service'
...
const getCollectingByName = async (req, res) => {
    try {
        ...
        const result = await collectingAdminService.getCollectingByName(name)
        res.status(HttpStatusCode.OK).json(result)
    } catch (error) {
        res.status(HttpStatusCode.INTERNAL_SERVER).json({
            error: error.message
...
const getFullCollection = async (req, res) => {
    try {
        const result = await collectingAdminService.getFullCollection()
        res.status(HttpStatusCode.OK).json(result)
    } catch (error) {
        res.status(HttpStatusCode.INTERNAL_SERVER).json({
            error: error.message
...
export const collectingAdminController = { 
    getFullCollection,
    getCollectingByName
    // getFullUser, 
    // update 
