...
const pcGamingCollectionName = 'pc-gaming'
const pcGamingCollectionSchema = Joi.object({
    ...
    collection: Joi.string().default(pcGamingCollectionName),
...
const validateSchema = async (data) => {
    return await pcGamingCollectionSchema.validateAsync(data, { abortEarly: false }) // Hiển thị đầy đủ lỗi nếu trong trường data có 2 field trở lên bị lỗi
}

const createNew = async (data) => {
    try {
        const value = await validateSchema(data)
        // const dataFind = await getDB().collection(pcGamingCollectionName).aggregate([
        //     {
        //         $match: {
        //             email: value.email,
        //             _destroy: false
        ...
        // } else {
        const result = await getDB().collection(pcGamingCollectionName).insertOne(value)
        return result
        // }
    } catch (error) {
...
const findOneById = async (id) => {
    try {
        const result = await getDB().collection(pcGamingCollectionName).findOne({ _id: ObjectId(id) })
        return result
    } catch (error) {
        throw new Error(error)
    }
...
const update = async (src, data) => {
    try {
        ...
        const srcGet = src
        const updateUser = await getDB().collection(pcGamingCollectionName).findOneAndUpdate(
            { src: srcGet },
            { $set: updateData },
...
const getFullPcGamingCollecting = async (data, role) => {
    try {
        ...
        if (role.role === 'CEO' || role.role === 'MANAGEMENT') {
            ...
            const result = await getDB().collection(pcGamingCollectionName).find().limit(perPage).skip((perPage * page) - perPage).toArray()
            const resultTotal = await getDB().collection(pcGamingCollectionName).find().toArray()
...
const getFullPcGamingInformationAdmin = async (src) => {
    try {
        const result = await getDB().collection(pcGamingCollectionName).aggregate([
            {
                $match: {
                    src: src,
                    _destroy: false
...
const getSearchPcGamingInformation = async (data) => {
    try {
        ...
        const resultTotal = await getDB().collection(pcGamingCollectionName).aggregate([
            {
                $match: {
                    nameProduct: { $regex: new RegExp(data.nameProduct, 'i') },
                    category: filteredCategory.length > 0 ? { $all: filteredCategory } : { $exists: true },
...
export const pcGamingCollectingModel = {
    createNew,
