...
const appleCollectionName = 'apple'
const appleCollectionSchema = Joi.object({
    ...
    collection: Joi.string().default(appleCollectionName),
...
const validateSchema = async (data) => {
    return await appleCollectionSchema.validateAsync(data, { abortEarly: false }) // Hiển thị đầy đủ lỗi nếu trong trường data có 2 field trở lên bị lỗi
}

const createNew = async (data) => {
    try {
        const value = await validateSchema(data)
        // const dataFind = await getDB().collection(appleCollectionName).aggregate([
        //     {
        //         $match: {
        //             email: value.email,
        //             _destroy: false
        ...
        // } else {
        const result = await getDB().collection(appleCollectionName).insertOne(value)
        return result
        // }
    } catch (error) {
...
const findOneById = async (id) => {
    try {
        const result = await getDB().collection(appleCollectionName).findOne({ _id: ObjectId(id) })
        return result
    } catch (error) {
        throw new Error(error)
    }
...
const update = async (src, data) => {
    try {
        ...
        const srcGet = src
        const updateUser = await getDB().collection(appleCollectionName).findOneAndUpdate(
            { src: srcGet },
            { $set: updateData },
...
const getFullAppleCollecting = async (data, role) => {
    try {
        ...
        if (role.role === 'CEO' || role.role === 'MANAGEMENT') {
            ...
            const result = await getDB().collection(appleCollectionName).find().limit(perPage).skip((perPage * page) - perPage).toArray()
            const resultTotal = await getDB().collection(appleCollectionName).find().toArray()
...
const getFullAppleInformationAdmin = async (src) => {
    try {
        const result = await getDB().collection(appleCollectionName).aggregate([
            {
                $match: {
                    src: src,
                    _destroy: false
...
const getSearchAppleInformation = async (data) => {
    try {
        ...
        const resultTotal = await getDB().collection(appleCollectionName).aggregate([
            {
                $match: {
                    nameProduct: { $regex: new RegExp(data.nameProduct, 'i') },
                    category: filteredCategory.length > 0 ? { $all: filteredCategory } : { $exists: true },
...
export const appleCollectingModel = {
    createNew,
