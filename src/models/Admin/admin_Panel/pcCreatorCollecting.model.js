...
const pcCreatorCollectionName = 'pc-creator'
const pcCreatorCollectionSchema = Joi.object({
    ...
    collection: Joi.string().default(pcCreatorCollectionName),
...
const validateSchema = async (data) => {
    return await pcCreatorCollectionSchema.validateAsync(data, { abortEarly: false }) // Hiển thị đầy đủ lỗi nếu trong trường data có 2 field trở lên bị lỗi
}
...
const createNew = async (data) => {
    try {
        ...
        // } else {
        const result = await getDB().collection(pcCreatorCollectionName).insertOne(value)
        return result
        // }
    } catch (error) {
...
const findOneById = async (id) => {
    try {
        const result = await getDB().collection(pcCreatorCollectionName).findOne({ _id: ObjectId(id) })
        return result
    } catch (error) {
        throw new Error(error)
    }
...
const update = async (src, data) => {
    try {
        ...
        const srcGet = src
        const updateUser = await getDB().collection(pcCreatorCollectionName).findOneAndUpdate(
            { src: srcGet },
            { $set: updateData },
...
const getFullPcCreatorCollecting = async (data, role) => {
    try {
        ...
        if (role.role === 'CEO') {
            ...
            const resultTotal = await getDB().collection(pcCreatorCollectionName).find().toArray()
...
const getFullPcCreatorInformationAdmin = async (src) => {
    try {
        const result = await getDB().collection(pcCreatorCollectionName).aggregate([
            {
                $match: {
                    src: src,
                    _destroy: false
...
const getSearchPcCreatorInformation = async (data) => {
    try {
        ...
        const resultTotal = await getDB().collection(pcCreatorCollectionName).aggregate([
            {
                $match: {
                    nameProduct: { $regex: new RegExp(data.nameProduct, 'i') },
                    category: filteredCategory.length > 0 ? { $all: filteredCategory } : { $exists: true },
...
export const pcCreatorCollectingModel = {
    createNew,
