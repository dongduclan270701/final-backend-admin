...
const pcCompanyCollectionName = 'pc-company'
const pcCompanyCollectionSchema = Joi.object({
    ...
    collection: Joi.string().default(pcCompanyCollectionName),
    ...
    showInMonth: Joi.array().default([]),
})

const validateSchema = async (data) => {
    return await pcCompanyCollectionSchema.validateAsync(data, { abortEarly: false }) // Hiển thị đầy đủ lỗi nếu trong trường data có 2 field trở lên bị lỗi
}

const createNew = async (data) => {
    try {
        const value = await validateSchema(data)
        // const dataFind = await getDB().collection(pcCompanyCollectionName).aggregate([
        //     {
        //         $match: {
        //             email: value.email,
        //             _destroy: false
        ...
        // } else {
        const result = await getDB().collection(pcCompanyCollectionName).insertOne(value)
        return result
        // }
    } catch (error) {
...
const findOneById = async (id) => {
    try {
        const result = await getDB().collection(pcCompanyCollectionName).findOne({ _id: ObjectId(id) })
        return result
    } catch (error) {
        throw new Error(error)
    }
...
const update = async (src, data) => {
    try {
        ...
        const srcGet = src
        const updateUser = await getDB().collection(pcCompanyCollectionName).findOneAndUpdate(
            { src: srcGet },
            { $set: updateData },
...
const getFullPcCompanyCollecting = async (data, role) => {
    try {
        ...
        if (role.role === 'CEO') {
            ...
            const resultTotal = await getDB().collection(pcCompanyCollectionName).find().toArray()
...
const getFullPcCompanyInformationAdmin = async (src) => {
    try {
        const result = await getDB().collection(pcCompanyCollectionName).aggregate([
            {
                $match: {
                    src: src,
                    _destroy: false
...
const getSearchPcCompanyInformation = async (data) => {
    try {
        ...
        const resultTotal = await getDB().collection(pcCompanyCollectionName).aggregate([
            {
                $match: {
                    nameProduct: { $regex: new RegExp(data.nameProduct, 'i') },
                    category: filteredCategory.length > 0 ? { $all: filteredCategory } : { $exists: true },
...
export const pcCompanyCollectingModel = {
    createNew,
